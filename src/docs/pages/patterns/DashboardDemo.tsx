...
function exportPDF(data: Row[], filters: Record<string,string|null>, totals: {valor:string}){
  ...
  pageRows.forEach((pRows,pi)=>{
    ...
    s+="BT\n0.318 0.38 0.427 rg\n";
    s+=`${mx} ${fy-12} Td /F1 7 Tf (Total: R$ ${totals.valor}  |  ${data.length} requisicoes  |  DS-FIPS v2.0 - Ferrovia Interna do Porto de Santos) Tj\n`;
    s+="ET\n";
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:Fn.body,color:C.cinzaEscuro}}>
      ...
      <div style={{padding:mob?"16px 12px 32px":"24px 40px 48px",maxWidth:1200}}>
        ...
        <div style={{textAlign:"center",padding:"24px 0 0",marginTop:24}}>
          <span style={{fontSize:11,color:C.cinzaChumbo,letterSpacing:".5px",fontFamily:Fn.title,fontWeight:400}}>DS-FIPS v2.0 · Ferrovia Interna do Porto de Santos · Excelência sobre trilhos · {new Date().getFullYear()}</span>
        </div>
      </div>
    </div>
  );
